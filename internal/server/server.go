package server
import ("encoding/json";"log";"net/http";"github.com/stockyard-dev/stockyard-prospector/internal/store")
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){
    ...
    if v:=r.URL.Query().Get("stage");v!=""{filters["stage"]=v}
    if q!=""||len(filters)>0{wj(w,200,map[string]any{"deals":oe(s.db.Search(q,filters))});return}
    wj(w,200,map[string]any{"deals":oe(s.db.List())})
}
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{items:=s.db.List();if len(items)>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/prospector/");return}};var e store.Deal;json.NewDecoder(r.Body).Decode(&e);if e.Name==""{we(w,400,"name required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)update(w http.ResponseWriter,r *http.Request){
    ...
    var patch store.Deal;json.NewDecoder(r.Body).Decode(&patch);patch.ID=existing.ID;patch.CreatedAt=existing.CreatedAt
...
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"prospector","deals":s.db.Count()})}
